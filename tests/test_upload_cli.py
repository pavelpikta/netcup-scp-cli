...
from netcup_cli.cli.main import cli
...
def test_images_upload_cli(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    with (
        patch("netcup_cli.cli.users_resources_cmd.resolve_user_id", return_value=7),
        patch("netcup_cli.cli.users_resources_cmd.user_image_upload") as upload,
    ):
...
def test_isos_upload_cli(tmp_path: Path) -> None:
    ...
    with (
        patch("netcup_cli.cli.users_resources_cmd.resolve_user_id", return_value=7),
        patch("netcup_cli.cli.users_resources_cmd.user_iso_upload") as upload,
    ):
